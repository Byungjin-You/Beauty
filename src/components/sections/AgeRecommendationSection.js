...
import { ageRecommendations, ageGroups } from "../../data/age-recommendations";

const AgeRecommendationSection = () => {
  ...
  return (
    <div className="flex flex-col mt-[24px] tablet:mt-[32px] desktop:mt-[40px]">
      ...
      {/* 나이대 필터 - 의사/병원 페이지 의사상담 필터와 동일한 디자인 */}
      <div className="tablet:mx-[-24px] mx-[-16px] tablet:px-[24px] px-[16px] tablet:pt-[24px] pt-[20px] tablet:pb-[16px] pb-[12px] z-10">
        <div className="flex items-center gap-[8px]">
          {ageGroups.map((ageGroup) => (
            <button 
              key={ageGroup.id}
              className={`flex flex-none items-center gap-[4px] border-[1.5px] rounded-[8px] h-[32px] px-[8px] transition-colors ${
            ...
            >
              <span 
                ...
                style={{ 
                  ...
                  color: selectedAgeGroup === ageGroup.id ? 'inherit' : 'rgb(49, 49, 66)'
                }}
              >
                check
              </span>
      ...
      >
        <ol className="flex flex-col flex-wrap h-[258px] gap-x-[16px]">
          {ageRecommendations.slice(0, 9).map((item, index) => (
      ...
      <div className="tablet:mx-[-24px] mx-[-16px] tablet:px-[24px] px-[16px] mt-[16px]">
        <button 
          ...
          onClick={() => {
            const selectedGroup = ageGroups.find(group => group.id === selectedAgeGroup);
            window.location.href = `/rankings?age=${selectedAgeGroup}`;
          }}
        >
          {ageGroups.find(group => group.id === selectedAgeGroup)?.label} 전체보기
...
export default AgeRecommendationSection;
